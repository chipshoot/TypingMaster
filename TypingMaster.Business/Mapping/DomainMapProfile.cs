...
namespace TypingMaster.Business.Mapping
{
    public class DomainMapProfile : Profile
    {
        public DomainMapProfile()
        {
            ...
            // We use course name to determine the type of course to map to
            ...
            // Map from CourseDao to CourseDto
            CreateMap<CourseDao, CourseDto>()
                .ForMember(dest => dest.Id, opt => opt.MapFrom(src => src.Id))
                .ForMember(dest => dest.AccountId, opt => opt.MapFrom(src => src.AccountId))
                .ForMember(dest => dest.Name, opt => opt.MapFrom(src => src.Name))
                .ForMember(dest => dest.Type, opt => opt.MapFrom(src => Enum.Parse<TrainingType>(src.Type)))
                .ForMember(dest => dest.LessonDataUrl, opt => opt.MapFrom(src => src.LessonDataUrl))
                .ForMember(dest => dest.Lessons, opt => opt.Ignore()) // Lessons might need custom mapping
                .ForMember(dest => dest.Settings, opt => opt.MapFrom(src => new CourseSetting
                {
                    Minutes = src.SettingsJson.Minutes,
                    NewKeysPerStep = src.SettingsJson.NewKeysPerStep,
                    PracticeTextLength = src.SettingsJson.PracticeTextLength,
                ...
                .ForMember(dest => dest.Description, opt => opt.MapFrom(src => src.Description)); // Map from CourseDto to CourseDao (reverse mapping)

            CreateMap<CourseDto, CourseDao>()
                .ForMember(dest => dest.Id, opt => opt.MapFrom(src => src.Id))
                .ForMember(dest => dest.AccountId, opt => opt.MapFrom(src => src.AccountId))
                .ForMember(dest => dest.Name, opt => opt.MapFrom(src => src.Name))
                .ForMember(dest => dest.Type, opt => opt.MapFrom(src => ((int)src.Type).ToString()))
                .ForMember(dest => dest.LessonDataUrl, opt => opt.MapFrom(src => src.LessonDataUrl))
                .ForMember(dest => dest.Description, opt => opt.MapFrom(src => src.Description))
                .ForMember(dest => dest.SettingsJson, opt => opt.MapFrom(src => src.Settings == null
                    ? null
                    : new CourseSettingDao
                    {
                        Minutes = src.Settings.Minutes,
                        NewKeysPerStep = src.Settings.NewKeysPerStep,
                        PracticeTextLength = src.Settings.PracticeTextLength,
